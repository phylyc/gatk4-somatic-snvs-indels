...
## keep_germline: Also selects variants with the germline flag
...
## force_call_alleles, force_call_alleles_idx: optional vcf (and its index) of alleles
##      that are being called regardless of evidence.
...
## Further input args are available for memory and runtime assignment of the individual tasks.
## The default values have been optimized based on maximum used resources for more
## than 1000 patients of multi-sample calling, with up to 25 samples per patient.
##
## ** Workflow options **
...
##      Tumor-only cases generate ~50k variants, which, if split over less than
    ...
    if (run_cnn_scoring_model || run_funcotator) {
        ...
        scatter (sample in samples) {
            ...
            String? normal_sample_name = (
                if normal_is_present && !run_cnn_scoring_model && sample.is_tumor
